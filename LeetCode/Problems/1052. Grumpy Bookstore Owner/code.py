...
class Solution:
    def maxSatisfied(self, customers: List[int], grumpy: List[int], X: int) -> int:
        ...
        customer2 = customerX = sum(customers[i] * grumpy[i] for i in range(X))
        for i in range(X, N):
            customerX = customerX + customers[i] * grumpy[i] - customers[i - X] * grumpy[i - X]
            customer2 = max(customer2, customerX)
